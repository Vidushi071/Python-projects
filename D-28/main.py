...
def count_down(count):
    ...
    if count_sec < 10:# we are using dynamic typing (not available in all languages
        count_sec = f"0{count_sec}"

    canvas.itemconfig(timer_count, text=f"{count_min}:{count_sec}")
    if count > 0:
        global a
        a=window.after(1000, count_down, count - 1)
    else:
        start_timer()
        marks = ""
...
timer_count = canvas.create_text(100, 130, text="00:00", font=(FONT_NAME, 24, "bold"), fill="white")
...
check = Label( fg=GREEN , font=100, bg=YELLOW)
